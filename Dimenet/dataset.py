...
class make_datasets():
    ...
    def make_graph_datas(self, num_xtb_data, num_g09_data, load=False, random_state=1000, g09_data_cut=None, with_rdkit=False, rdkit_rank=10, to_class=False, class_num=5, cut_mode='qcut', onehot=False, xtb_state=0, g09_state=1):
        ...
        if not load:
            ...
            for xyz in tqdm(g09_xyz_list):
                state_attributes = [g09_state] # xtb=0, g09=1
        ...
        if not load:
            ...
            for xyz in tqdm(xtb_xyz_list):
                ...
                if key_name in xtb_keys:
                    state_attributes = [xtb_state] # xtb=0, g09=1
                    name = os.path.basename(xyz).split(".xyz")[0]
                    if with_rdkit:
                        try:
                            state_attributes = self.add_rdkit_descriptor(name, state_attributes, rdkit_data, onehot=onehot)
