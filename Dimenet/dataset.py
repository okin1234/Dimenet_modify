...
from .molecule import MolecularGraph

def data_to_onehot(rdkit_data):
...
def data_sampling(input_df=None, output_df=None, mode='qcut', sample_column=None, class_num=5):
    if mode=='cut':
...
def feature_to_class(xtb_df, g09_df, to_class=False, class_num=5, cut_mode='qcut'):
    xtb_features = ['cid']
    g09_features = ['cid']
    ...
    if to_class:
        ...
        for feature in xtb_features[1:]:
            final_df, class_counts = data_sampling(input_df=feature_df, output_df=final_df, mode=cut_mode, sample_column=feature, class_num=class_num)
    else:
        final_df = feature_df
    return final_df

def extract_rdkit_descriptor(importance_rank, to_class, class_num, cut_mode):
    lightgbm_importance = pd.read_csv('/home/rnwnsgud1234/4.BRL/12.MEGNET/Boosting_model/feature_importance/feature_importance_with_recalc_data.csv')
    selected_feature_list = lightgbm_importance['features'][:importance_rank].to_list()
    selected_feature_list.extend(['cid'])
    
    g09_df = pd.read_csv('/home/rnwnsgud1234/4.BRL/12.MEGNET/Boosting_model/training_2d_descriptor_data_rdkit.csv')
    xtb_df = pd.read_csv('/home/rnwnsgud1234/4.BRL/12.MEGNET/Boosting_model/xtb_rdkit_2d_descriptor_data.csv')
    
    rdkit_data = feature_to_class(xtb_df, g09_df, to_class=to_class, class_num=class_num, cut_mode=cut_mode)

    rdkit_data = rdkit_data[selected_feature_list]
    return rdkit_data

def add_rdkit_descriptor(name, state_attributes, rdkit_data, onehot=False):
    cid = int(name)
...
def make_graph_datas(num_xtb_data, num_g09_data, load=False, random_state=1000, g09_data_cut=None, with_rdkit=False, rdkit_rank=10, to_class=False, class_num=5, cut_mode='qcut', onehot=False):
    def xyz2graph(xyz_file, state_attributes):
        with open(xyz_file, 'r') as f:
            string = f.read()
        name = os.path.basename(xyz_file).split(".xyz")[0]
        mol = readstring('xyz', string)
        structure_data = molecule_graph.convert(mol, state_attributes=state_attributes)
        return name, structure_data

    ''' graph convert model defination '''
    # relaxed xyz file list
    xtb_xyz_list = glob("/home/rnwnsgud1234/4.BRL/12.MEGNET/Datasets/xtb/xyz_files/*.xyz")
    g09_xyz_list = glob("/home/rnwnsgud1234/4.BRL/12.MEGNET/Datasets/g09/relaxed_xyz/*.xyz")

    # graph converter defination
    molecule_graph = MolecularGraph()
    ...
    if not load:
        ...
        if with_rdkit==True:
            ...
            rdkit_data = extract_rdkit_descriptor(importance_rank=rdkit_rank, to_class=to_class, class_num=class_num, cut_mode=cut_mode)
        if onehot:
        ...
        for xyz in tqdm(g09_xyz_list):
            ...
            data_type = 'g09'
            if with_rdkit:
                state_attributes = add_rdkit_descriptor(name, state_attributes, rdkit_data, onehot=onehot)
    ...
    xtb_result_csv = pd.read_csv("/home/rnwnsgud1234/4.BRL/12.MEGNET/Datasets/xtb/xtb_crest_result_addit.csv")
    g09_result_csv = pd.read_csv("/home/rnwnsgud1234/4.BRL/12.MEGNET/Datasets/g09/g09_selected_mols_result.csv")
    
    ################################## for test ###############################
    #xtb_result_csv = xtb_result_csv[:100]
    ...
    ''' xtb error data 제거 '''
    xtb_error_cid_1 = pd.read_csv("/home/rnwnsgud1234/4.BRL/12.MEGNET/Datasets/xtb/error_cid_list.csv")
    xtb_error_cid_1 = xtb_error_cid_1['error_cid'].to_list()
    with open("/home/rnwnsgud1234/4.BRL/12.MEGNET/Datasets/xtb/charge_error_cid_list.pickle", 'rb') as fr:
    ...
    g09_target = {}
    print('g09 datas convert to target datas~~')
    ...
    xtb_target = {}
    print('xtb datas convert to target datas~~ (it may be doing for few minutes...)')
    ...
    #''' xtb_datas cleaning '''
    #xtb_key_list = xtb_target.keys()
    #for key, item in list(xtb_datas.items()):
    #    if key not in xtb_key_list:
    #        xtb_datas.pop(key, None) 
    ...
    if not load:
        ...
        ''' graph data save '''
        import pickle
        with open('xtb_datas_for_test.pickle','wb') as fw:
            pickle.dump(xtb_datas, fw)
    else:
        import pickle
        with open('xtb_datas_for_test.pickle','rb') as fr:
            xtb_datas = pickle.load(fr)
    print('The length of xtb_datas is', len(xtb_datas))
    print('The length of xtb_targets is', len(xtb_target))
